import bashProjects from './bash-projects.json';
import Image from 'next/image';
import styles from './BashProjects.module.css';

const PROJECTS_PER_PAGE = 6;

export function generateStaticParams() {
  const totalPages = Math.ceil(bashProjects.length / PROJECTS_PER_PAGE);
...
export default async function BashProjectsPage({ params }) {
  ...
  const currentProjects = bashProjects.slice(startIdx, endIdx);
  ...
  return (
    <div className={styles.bashProjectsBg}>
      <main className={styles.bashProjectsMain}>
        <h1 className={styles.bashProjectsTitle}>Bash Projects</h1>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', margin: '32px 0', marginTop: 'auto', paddingTop: '20px' }}>
          <a
            href={page > 1 ? `/devops-projects/bash-projects/page/${page - 1}` : '#'}
            style={{
          ...
          <a
            href={page < totalPages ? `/devops-projects/bash-projects/page/${page + 1}` : '#'}
            style={{
